...
def ProKeR(vecs, labels, val_features, val_labels, test_features, clip_weights, dataset, shots, seed, hp_selection, backbone='RN50'):
    ...
    if hp_selection != 'imagenet': # grid search for hyper-parameters
        ...
        if exists(f'./configs/{backbone}/configs_proker/{dataset}/seed_{seed}_shots{shots}.json'): # load best hp if already exists
            best_hp = json.load(open(f'./configs/{backbone}/configs_proker/{dataset}/seed{seed}_shots{shots}.json', 'r'))
            best_beta, best_lmbda = best_hp['beta'], best_hp['lmbda']
            print(f'Loaded best hp from ./configs/{backbone}/configs_proker/{dataset}/seed{seed}_shots{shots}.json: beta={best_beta}, lmbda={best_lmbda}')
            best_val_acc = None
          ...
          save_hps(best_hp, f'./configs/{backbone}/configs_proker/{dataset}', seed,shots) # save best hp
