...
class ImageNet():
    ...
    def __init__(self, cfg, root, num_shots, preprocess):
        ...
        train_preprocess = transforms.Compose([
                        transforms.RandomResizedCrop(size=224, scale=(0.08, 1.0), interpolation=transforms.InterpolationMode.BICUBIC),
        ...
        if num_shots != -1:
            ...
            if subsample == 'all':
                ...
                self.classnames = imagenet_classes[:m]
                # train
                split_by_label_dict = defaultdict(list)
                for i in range(len(self.train.imgs)):
                    split_by_label_dict[self.train.targets[i]].append(self.train.imgs[i])
                ...
                # test
                split_by_label_dict = defaultdict(list)
                for i in range(len(self.test.imgs)):
                    split_by_label_dict[self.test.targets[i]].append(self.test.imgs[i])
                imgs = []
                targets = []
                for label, items in split_by_label_dict.items():
                    imgs = imgs + items
